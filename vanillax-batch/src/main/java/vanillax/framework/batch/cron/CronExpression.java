...
/*
 * Referenced : https://github.com/zemiak/cron/blob/master/src/main/java/fc/cron/CronExpression.java
 */
import java.time.*;
 ...
 *
 * <P>
 * 주의 : 요일이 월요일(1)부터 시작된다! &quot;SUN-MON&quot; 은 허용되지 않는다. &quot;SUN,MON&quot;은 허용된다.
 * </P>
 * <P>
...
public class CronExpression {
    ...
    public CronExpression(final String expression, final boolean withSeconds) {
        if (expression == null) {
            throw new IllegalArgumentException("expression is null"); //$NON-NLS-1$
        }

        this.expression = expression;

        final int expectedParts = withSeconds ? 6 : 5;
        final String[] parts = expression.split("\\s+"); //$NON-NLS-1$
        if (parts.length != expectedParts) {
            throw new IllegalArgumentException(String.format("Invalid cron expression [%s], expected %s felt, got %s"
                    , expression, expectedParts, parts.length));
        }
    ...
    /**
     * afterTime 이후부터 dateTimeBarrier 전까지 범위내에서 다음 시간을 조회한다.
     * @param afterTime 비교할 이전 시간
     * @param dateTimeBarrier 비교범위의 제한 시간
     * @return 범위내의 다음 시간을 반환한다.
     */
    ...
    public boolean isNowOnTime(){
        ...
        ZonedDateTime oneSecondBefore = now.plusSeconds(-1).withNano(0);
        if(nextTimeAfter(oneSecondBefore).equals(now)){
            return true;
        }
        return false;
    }

    public ZonedDateTime nextTimeAfterFromNow() {
        ZonedDateTime now = ZonedDateTime.now().withNano(0);
        return nextTimeAfter(now);
    }

    public String getNextTime() {
        ZonedDateTime next = nextTimeAfterFromNow();
